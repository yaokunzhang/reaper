...
// This provides an implementation for the "cargo checker" subcommand.
// The checker subcommand is the same as "cargo check" but with three differences:
// 1) It implicitly adds the options "--cfg checker -Z always_encode_mir" to the rustc invocation.
// 2) It calls checker rather than rustc for all the targets of the current package.
...
const CARGO_CHECKER_HELP: &str = r#"Static analysis tool for Rust programs
...
pub fn main() {
    if std::env::args().any(|a| a == "--help" || a == "-h") {
        println!("{CARGO_CHECKER_HELP}");
        return;
    }
    match std::env::args().nth(1).as_ref().map(AsRef::<str>::as_ref) {
        Some(s) if s.ends_with("checker") || s.ends_with("checker.exe") => {
            // Get here for the top level cargo execution, i.e. "cargo checker".
        ...
        Some(s) if s.ends_with("rustc") || s.ends_with("rustc.exe") => {
            ...
            // execute rustc with CHECKER applicable parameters for dependencies and call CHECKER
            // to analyze targets in the current package.
            if std::env::args().any(|a| a == "--version" || a == "-V") {
                call_rustc();
                return;
            }
            call_rustc_or_checker();
        }
        Some(arg) => {
            eprintln!(
                "`cargo-checker` called with invalid first argument: {arg}; please only invoke this binary through `cargo checker`" 
            );
        }
        _ => {
            eprintln!(
                "`cargo-checker` called without first argument; please only invoke this binary through `cargo checker`"
            );
        }
    }
}
...
fn call_cargo_on_target(target: &Target, kind: &TargetKind) {
    ...
    for arg in args.by_ref() {
        ...
        // // 新增：避免用户手动指定的 message-format 参数冲突
    ...
    // println!("cargo args: {:?}", cmd.get_envs());

    // Force cargo to recompile all dependencies with CHECKER friendly flags
    cmd.env("RUSTFLAGS", "--cfg checker -Z always_encode_mir");

    // Replace the rustc executable through RUSTC_WRAPPER environment variable so that rustc
    // calls generated by cargo come back to cargo-checker.
    let path = std::env::current_exe().expect("current executable path invalid");
    cmd.env("RUSTC_WRAPPER", path);

    // Communicate the name of the root crate to the calls to cargo-checker that are invoked via
    // the RUSTC_WRAPPER setting.
    cmd.env("CHECKER_CRATE", target.name.replace('-', "_"));

    // Communicate the target kind of the root crate to the calls to cargo-checker that are invoked via
    // the RUSTC_WRAPPER setting.
    cmd.env("CHECKER_KIND", kind_str);

    // Set the tool chain to be compatible with checker
...
fn call_rustc_or_checker() {
    if let Some(crate_name) = get_arg_flag_value("--crate-name") {
        if let Ok(checker_crate) = std::env::var("CHECKER_CRATE") {
            if crate_name.eq(&checker_crate) {
                if let Ok(kind) = std::env::var("CHECKER_KIND") {
...
fn call_checker() {
    ...
    path.pop(); // remove the cargo_checker bit
    path.push("checker");
    if let Some(ext) = extension {
        path.set_extension(ext);
    }
    let mut cmd = Command::new(path);
    // println!("checker args: {:?}", cmd);
    cmd.args(std::env::args().skip(2));
    let exit_status = cmd
        .spawn()
        .expect("could not run checker")
        .wait()
