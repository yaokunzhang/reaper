    ...
    ApronAbstractDomain<DomainType>: GetManagerTrait,
{
    ...
    pub fn check_assert_condition(
        ...
        // 使用 catch_unwind 和 unwrap_or_else 简化错误处理
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            debug!("z3_cond_expr: {:?}", z3_cond_expr);
            ...
            match solver.solve_expression(&z3_cond_expr) {
                ...
                SmtResult::Unknown => CheckerResult::Warning,
            }
        }))
        .unwrap_or_else(|e| {
            // 记录 panic 信息
            if let Some(msg) = e.downcast_ref::<&str>() {
                debug!("Z3求解器发生panic: {}", msg);
            } else if let Some(msg) = e.downcast_ref::<String>() {
                debug!("Z3求解器发生panic: {}", msg);
            } else {
                debug!("Z3求解器发生未知panic");
            }
    ...
    fn check_overflow(
        ...
        match &value.expression {
            ...
            _ => {
                unreachable!("Overflow operand is not a path {:?}", value.expression);
            }
        }
    }
}
