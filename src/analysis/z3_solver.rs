...
impl Z3Solver {
    ...
    pub fn convert_to_bool_sort(&self, exp: Z3Expression) -> Z3Expression {
        debug!("convert_to_bool_sort: {:?}", exp);
        unsafe {
    ...
    pub fn get_symbolic_as_z3_expression(&self, symbolic_value: &SymbolicValue) -> Z3Expression {
        debug!("get_symbolic_as_z3_expression {:?}", symbolic_value);
        use Expression::*;
        let res = match &symbolic_value.expression {
            Drop(..) => unimplemented!(),
            ...
            Cast {operand, ..} => self.get_symbolic_as_z3_expression(operand),
            Offset { .. }
            | HeapBlock { .. }
            | Join { .. }
            // | Offset { .. }
            | Reference(..)
            | Widen { .. } => {
                unimplemented!("Cannot cast symbolic value into Z3 ast{:?}", symbolic_value.expression);
            }
        };
        debug!("finish ");
        res
    ...
    fn make_comparison(
        ...
        debug!("make_comparison: lhs == {:?}", lhs);
        debug!("make_comparison: rhs == {:?}", rhs);
        let left = self.get_symbolic_as_z3_expression(lhs);
        let right = self.get_symbolic_as_z3_expression(rhs);
        // trigger panic, if one of ops is any_sort, and the other not;
        unsafe {
            let left_sort = z3_sys::Z3_get_sort(self.z3_context, left);
            let right_sort = z3_sys::Z3_get_sort(self.z3_context, right);
            let left_sort_kind = z3_sys::Z3_get_sort_kind(self.z3_context, left_sort);
            let right_sort_kind = z3_sys::Z3_get_sort_kind(self.z3_context, right_sort);
            if left_sort_kind != right_sort_kind {
                return z3_sys::Z3_mk_false(self.z3_context);
            }
        }
        let res = unsafe { op(self.z3_context, left, right) };
        debug!("make_comparison finish!");
        res
    }
