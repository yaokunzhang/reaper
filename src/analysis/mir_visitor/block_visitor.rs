    ...
    ApronAbstractDomain<DomainType>: GetManagerTrait,
{
    ...
    pub body_visitor: &'b mut BodyVisitor<'tcx, 'a, 'compiler, DomainType>,
    ...
    ApronAbstractDomain<DomainType>: GetManagerTrait,
{
    ...
    pub fn new(
        body_visitor: &'b mut BodyVisitor<'tcx, 'a, 'compiler, DomainType>,
    ...
    /// 判断 RHS 的 Rvalue 是否为 move（存在 Operand::Move）
    fn rvalue_is_move(&self, rvalue: &mir::Rvalue<'tcx>) -> bool {
        ...
        match rvalue {
            ...
            | Rvalue::Cast(_, op, _)
            | Rvalue::UnaryOp(_, op)
            | Rvalue::ShallowInitBox(op, _) => matches!(op, Operand::Move(..)),

            Rvalue::BinaryOp(_, box (l, r)) => {
                matches!(l, Operand::Move(..)) || matches!(r, Operand::Move(..))
            }

            Rvalue::Aggregate(_, ops) => ops.iter().any(|op| matches!(op, Operand::Move(..))),

            // 明确是 copy 语义
            Rvalue::CopyForDeref(_) => false,

            // 不涉及所有权转移（借用、常量、长度、标签等）
            Rvalue::Ref(..)
            | Rvalue::RawPtr(..)
            | Rvalue::Discriminant(..)
            | Rvalue::NullaryOp(..)
    ...
    fn visit_assign(&mut self, place: &mir::Place<'tcx>, rvalue: &mir::Rvalue<'tcx>) {
        ...
        info!(
            "rvalue {:?} is moved? {:?}",
            rvalue,
            self.rvalue_is_move(rvalue)
        );
        if self.rvalue_is_move(rvalue) {
            use crate::analysis::ownership::ownership_state::OwnershipState;

            // 默认 move 之后是 Owned，但如果源是 Uinit，则保持 Uinit 语义
            let mut new_state = OwnershipState::Owned;

            if let mir::Rvalue::Use(mir::Operand::Move(src_place)) = rvalue {
                let src_path = self.get_path_for_place(src_place);
                let src_states = self
                    .body_visitor
                    .state
                    .ownership_domain
                ...
                if src_states.contains(&OwnershipState::Uinit) {
                    new_state = OwnershipState::Uinit;
                }
            }
        ...
        match &kind {
            rustc_middle::ty::ConstKind::Param(ParamConst { index, .. }) => {
                if let Some(gen_args) = self.body_visitor.type_visitor.generic_arguments {
                    // println!("gen_args is {:?}", gen_args);
                    if let Some(arg_val) = gen_args.as_ref().get(*index as usize) {
                        // let res = self.visit_constant(&arg_val.expect_const());
                        if arg_val.expect_const() == *literal {
            ...
            rustc_middle::ty::ConstKind::Value(value) => {
                ...
                if value.valtree.try_to_scalar_int().is_some() {
                    let scalar_int = &value.valtree.try_to_scalar_int().unwrap();
                    let (data, size) = Self::get_scalar_int_data(scalar_int);
                    result = self.get_constant_from_scalar(*lty, data, size);
                } else {
                    let val_tree = &value.valtree;
    ...
    //                 rustc_abi::Size::from_bytes(start as u64),
    //             ),
    //             rustc_abi::Size::from_bytes(slice_len as u64),
    //         )
    //         .unwrap();
    ...
    //                         &self.body_visitor.context.tcx,
    //                         *ptr,
    //                         rustc_abi::Size::from_bytes(num_bytes),
    //                     )
    //                     .unwrap();
    ...
    // 在现有的 visit_return 函数中添加 double-free 检查
    fn visit_return(&mut self) {
        debug!("Visiting return at block: {:?}", self.current_block);
        self.body_visitor.result_blocks.insert(self.current_block);

        // 在函数返回前检查 double-free 和内存泄漏
        let errors = self.body_visitor.state.ownership_domain.check_double_free();
        ...
        for error in errors {
            let warning = self
                ...
                .session
                .dcx()
                .struct_span_warn(self.body_visitor.current_span, error.message);
            warning.emit();
        }
    }
    ...
    fn visit_drop(
        ...
        // 检查所有权不变量
        self.check_ownership_invariant_for_drop(&dropped_path);

        // 只将当前 dropped_path 的状态设置为 {Dropped}
        // 使用 set_state 而不是 add_state，确保状态集合中只有 Dropped
        self.body_visitor
            .state
            .ownership_domain
            .set_state(dropped_path.clone(), OwnershipState::Dropped);

        info!("Set {:?} state to {{Dropped}}", dropped_path);
        ...
        info!("=== Finished drop handling ===");
    }

    /// 检查 Invariant I: Ownership & Aliasing Validity (E-DoubleFree)
    ///
    /// 根据论文规则：
    /// ι = drop(x), ∃o′ ∈ σ_A.alias(x). σ_O(o′) ∩ InvalidSet ≠ ∅
    /// where InvalidSet = {Dropped, Moved}
    fn check_ownership_invariant_for_drop(&mut self, dropped_path: &Rc<Path>) {
        info!("=== Checking Ownership Invariant for drop ===");
        info!("Dropped path: {:?}", dropped_path);

        // Step 1: 获取别名集合 σ_A.alias(x)
        let alias_group = self
            .body_visitor
            .state
            .ownership_domain
            .shared_resources
            .find_group(dropped_path);
        ...
        // Step 2: 检查别名集合中是否存在无效状态
        // InvalidSet = {Dropped, Moved}
        let mut invalid_aliases = Vec::new();
        ...
        for alias_path in &alias_group {
            let states = self
                ...
                .state
                .ownership_domain
                .get_states(alias_path);

            // 检查 σ_O(o′) ∩ InvalidSet ≠ ∅
            if states.contains(&OwnershipState::Dropped) || states.contains(&OwnershipState::Moved)
            {
                invalid_aliases.push((alias_path.clone(), states.clone()));
                warn!(
                    "Found invalid alias: {:?} with states {:?}",
                    alias_path, states
                );
            }
        }

        // Step 3: 如果存在无效别名，直接发出错误
        if !invalid_aliases.is_empty() {
            let error_details: Vec<String> = invalid_aliases
                .iter()
            ...
            let warning = self.body_visitor.context.session.dcx().struct_span_warn(
                ...
                format!(
                    "[Checker] Provably error: Double Free - attempting to drop {:?}, \
                     but {} aliased path(s) are in invalid states: [{}]",
                    dropped_path,
                    invalid_aliases.len(),
                    error_details.join(", ")
                ),
            ...
            error!(
                "Double-free detected for {:?}: {} invalid alias(es)",
                dropped_path,
                invalid_aliases.len()
            );
        } else {
            info!("Ownership invariant check passed for {:?}", dropped_path);
        }

        info!("=== Finished Ownership Invariant check ===");
    }
    ...
    fn visit_aggregate(
        ...
        match aggregate_kind {
            ...
            mir::AggregateKind::RawPtr(ty, mutbl) => {
                ...
                self.visit_use(thin_pointer_path, &operands[FieldIdx::from_usize(0)]);
    ...
    fn visit_nullary_op(
        ...
        let value = match null_op {
            ...
            mir::NullOp::ContractChecks => todo!(),
        };
    ...
    fn bin_op_is_with_overflow(&mut self, bin_op: mir::BinOp) -> bool {
        match bin_op {
            mir::BinOp::AddWithOverflow
            | mir::BinOp::MulWithOverflow
            | mir::BinOp::SubWithOverflow => true,
            _ => false,
        }
    }
    ...
    fn visit_binary_op(
        ...
        // 检查操作数是否是未初始化的
        self.check_operand_uninit(left_operand);
        self.check_operand_uninit(right_operand);

        // For arithmetic binary operators, handle by numerical domain
        if let Some(op) = self.bin_op_to_apron_bin_op(bin_op) {
            let mut result_path = path.clone();
            if self.bin_op_is_with_overflow(bin_op) {
                // path变更为一个新的path.0，因为with_overflow时，返回值为（value, if_overflow）
                result_path = Path::new_field(path.clone(), 0).refine_paths(&self.state());
            }
            match (left_operand, right_operand) {
                ...
                _ => {
                    ...
                    self.body_visitor
                        .state
                        .numerical_domain
                        .apply_bin_op_place_place(op, &left_path, &right_path, &result_path);
                }
            }
        }
    ...
    fn visit_const_value(&mut self, val: ConstValue<'tcx>, lty: Ty<'tcx>) -> Rc<SymbolicValue> {
        info!("visit_const_value val: {:?} lty: {:?}", val, lty);
        match val {
            ...
            ConstValue::Scalar(Scalar::Ptr(ptr, _size)) => {
                    ...
                    .try_get_global_alloc(ptr.provenance.alloc_id())
                {
                    ...
                    Some(GlobalAlloc::Function { instance }) => {
                        ...
                        info!(
                            "func_name is {}",
                            self.body_visitor.context.tcx.def_path_str(def_id)
                        );
                        let args = self.body_visitor.type_visitor.specialize_generic_args(
                            instance.args,
                            &self.body_visitor.type_visitor.generic_argument_map,
    ...
    fn check_operand_uninit(&mut self, operand: &mir::Operand<'tcx>) {
        match operand {
            mir::Operand::Copy(place) | mir::Operand::Move(place) => {
                let path = self.visit_place(place);
                if !self
                    ...
                    .can_use(&path)
                {
                    ...
                    if states.contains(&crate::analysis::ownership::ownership_state::OwnershipState::Uinit)
                    {
                        let warning = self
                            .body_visitor
                            .context
                            ...
                            .struct_span_warn(
                                self.body_visitor.current_span,
                                "[Checker] Provably unsafe error: use of uninitialized memory",
                            );
                        warning.emit();
                    }
                }
    ...
    fn visit_operand_place(&mut self, place: &mir::Place<'tcx>) -> Rc<SymbolicValue> {
        ...
        if !is_uninit {
            ...
            if root_path != path {
                let root_states = self
                    .body_visitor
                    .state
                    .ownership_domain
                    .get_states(&root_path);
                
                if root_states.contains(&crate::analysis::ownership::ownership_state::OwnershipState::Uinit) {
                    is_uninit = true;
                }
            }
        }
        ...
        if self.body_visitor
            .state
            .ownership_domain
            .is_forgotten_or_root_forgotten(&path)
        {
            let root_path = Path::root(&path);
            let warning = self
                .body_visitor
                ...
                .struct_span_warn(
                    self.body_visitor.current_span,
                    format!(
                        "[Checker] Provably unsafe error: accessing field/sub-path {:?} of a forgotten object {:?} - accessing memory after mem::forget is undefined behavior",
                        path, root_path
                    ),
                );
            warning.emit();
    ...
    fn visit_address_of(&mut self, path: Rc<Path>, place: &mir::Place<'tcx>) {
        ...
        if self.body_visitor
            .state
            .ownership_domain
            .is_forgotten_or_root_forgotten(&value_path)
        {
            let root_path = Path::root(&value_path);
            let warning = self
                .body_visitor
                .context
                .session
                ...
                .struct_span_warn(
                    self.body_visitor.current_span,
                    format!(
                        "[Checker] Provably unsafe error: taking address of field/sub-path {:?} of a forgotten object {:?} - accessing memory after mem::forget is undefined behavior",
    ...
    pub fn visit_used_copy(&mut self, target_path: Rc<Path>, place: &mir::Place<'tcx>) {
        ...
        } = &rpath.value
        {
            if **selector == PathSelector::Deref {
                ...
                let actual_path = if let Some(val) = self.body_visitor.state.value_at(&refined_qualifier) {
                    match &val.expression {
                        Expression::Reference(path) => {
                            path.clone()
                        }
                ...
                if states.contains(&crate::analysis::ownership::ownership_state::OwnershipState::Uinit) {
                    let warning = self
                        ...
                        .struct_span_warn(
                            self.body_visitor.current_span,
                            "[Checker] Provably unsafe error: dereferencing pointer to uninitialized memory",
                        );
                    warning.emit();
                    info!("visit_used_copy - detected dereference of pointer to uninitialized memory, actual_path: {:?}", actual_path);
                }
            }
        }
