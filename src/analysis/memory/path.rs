...
// 不使用宏反而不会让apron_domain中的map失效。
// 原因： `Ord` (and `PartialOrd`) implementation should be consistent with `PartialEq` and `Eq`.
...
impl Path {
    pub fn root(path: &Rc<Path>) -> Rc<Path> {
        match &path.value {
            PathEnum::QualifiedPath { qualifier, .. } => Self::root(qualifier),
            _ => path.clone(),
        }
    }
