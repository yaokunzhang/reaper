...
impl SymbolicDomain {
    ...
    pub fn rename(&mut self, old_path: &Rc<Path>, new_path: &Rc<Path>) {
        info!("=== SymbolicDomain::rename ===");
        info!("rename - old_path: {:?}", old_path);
        info!("rename - new_path: {:?}", new_path);

        // 1. 处理直接映射
        if let Some(value) = self.value_map.remove(old_path) {
            info!("rename - moved direct mapping");
        ...
        for (path, value) in &self.value_map {
            let needs_update = match &value.expression {
                Expression::Reference(p)
                | Expression::Variable { path: p, .. }
                | Expression::Numerical(p) => p == old_path,
                _ => false, // 可以扩展以处理更复杂的表达式
            };

            if needs_update {
                info!(
                    "rename - updating reference in {:?}: {:?} -> {:?}",
                    path, old_path, new_path
                );
                let new_expression = match &value.expression {
                    Expression::Reference(_) => Expression::Reference(new_path.clone()),
                    Expression::Variable { var_type, .. } => Expression::Variable {
                        path: new_path.clone(),
                        var_type: var_type.clone(),
                    },
                    Expression::Numerical(_) => Expression::Numerical(new_path.clone()),
                    _ => value.expression.clone(),
                };

                let new_value = Rc::new(SymbolicValue {
                    expression: new_expression,
                    expression_size: value.expression_size, // 使用正确的字段名
                });
        ...
        for (path, new_value) in updates {
            self.value_map.insert(path, new_value);
            info!("rename - applied update to symbolic value");
        }

        info!("=== Finished SymbolicDomain::rename ===");
    }
